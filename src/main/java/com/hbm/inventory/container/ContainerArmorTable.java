...
public class ContainerArmorTable extends Container {
			...
			this.addSlotToContainer(new Slot(inventory, inventory.getSizeInventory() - 1 - i, -17 + 22, 36 + i * 18) {
				...
				public boolean isItemValid(ItemStack stack) {
					if(stack == ItemStack.EMPTY)
						return false;
					return stack.getItem().isValidArmor(stack, VALID_EQUIPMENT_SLOTS[k], player);
				}

				@Override
				@SideOnly(Side.CLIENT)
				public String getSlotTexture() {
					return ItemArmor.EMPTY_SLOT_NAMES[ItemArmor.EMPTY_SLOT_NAMES.length - 1 - k];
				}
			});
		}
	...
	public ItemStack transferStackInSlot(EntityPlayer p_82846_1_, int par2) {
		...
		if(var4 != null && var4.getHasStack()) {
			...
			if(par2 <= ArmorModHandler.MOD_SLOTS) {
				if(par2 != ArmorModHandler.MOD_SLOTS || !mergeItemStack(var5, ArmorModHandler.MOD_SLOTS + 1, ArmorModHandler.MOD_SLOTS + 5, false))
					if(!this.mergeItemStack(var5, ArmorModHandler.MOD_SLOTS + 5, this.inventorySlots.size(), true))
						return ItemStack.EMPTY;

				var4.onTake(p_82846_1_, var3);
			} else {
	...
	public class UpgradeSlot extends Slot {
		...
		@Override
		public boolean isItemValid(ItemStack stack) {
			return armor.getStackInSlot(0) != ItemStack.EMPTY && ArmorModHandler.isApplicable(armor.getStackInSlot(0), stack) && ((ItemArmorMod)stack.getItem()).type == this.slotNumber;
		}
